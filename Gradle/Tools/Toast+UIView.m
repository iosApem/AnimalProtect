...
static const CGFloat CSToastOpacity             = 0.65;
...
- (UIView *)makeToast:(NSString *)message duration:(CGFloat)interval  position:(id)position title:(NSString *)title image:(UIImage *)image {
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        } else if([point caseInsensitiveCompare:@"center_bottom"] == NSOrderedSame) {
            
            CGFloat centerY = self.bounds.size.height / 2;
            CGFloat bottomY = (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding;
            
            return CGPointMake(self.bounds.size.width / 2, (centerY + bottomY)/2);
        }
